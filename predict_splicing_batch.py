...
def plot_sse_density(all_data_df: pd.DataFrame, group_by: str, output_fn: str = None, log_fn=print):
        ...
        if output_fn is not None:
            ...
            plt.close()
            log_fn(f"Saved density plot to {output_fn}")
        else:
            plt.show()
...
print(f"Per-batch results will be saved to: {per_batch_dir}")
...
# Collect all data for final aggregated plots
# Use temporary files to avoid memory buildup
...
if os.path.exists(all_splice_predictions_file):
    ...
    for org in final_splice_df['organism'].unique():
        org_df = final_splice_df[final_splice_df['organism'] == org]
        ...
        for i in range(5):
            ...
            y_true = (org_df['label'].values == i).astype(int)
            y_scores = org_df[f'pred_{i}'].values
        ...
        plt.xlabel("Recall")
        plt.ylabel("Precision")
        plt.title(f"Final PR Curve - {org} Splice Site Classification")
        plt.legend(loc='lower left')
        plt.grid()
        plt.savefig(f"{output_dir}/splice_site_pr_curve_final_{org}.png", dpi=150)
        plt.close()
        print(f"  Saved: {output_dir}/splice_site_pr_curve_final_{org}.png")
